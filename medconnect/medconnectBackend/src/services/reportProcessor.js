    ...
    try {
        ...
        if (reportFiles && reportFiles.length > 0) {
            for (const file of reportFiles) {
                ...
                if (!textExtracted && (file.mimetype.startsWith('image/') || !reportContent || reportContent.trim().length === 0)) {
                    // Use Tesseract.js for OCR
                    try {
                        const { data: { text } } = await Tesseract.recognize(file.path, 'eng');
                        reportContent = text;
                        textExtracted = true;
                        console.log(`Extracted text from image or fallback using Tesseract for ${file.originalname}`);
                    } catch (ocrError) {
                        console.error(`Error extracting text with Tesseract from ${file.originalname}:`, ocrError);
                    ...
                    try {
                        ...
                        console.error(`Error reading file as text for ${file.originalname}:`, txtError);
                        reportContent = `Unknown Content from ${file.originalname}`;
                    }
                }
